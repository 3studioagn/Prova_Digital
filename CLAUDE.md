...
| **v4.0 W3 — C12 Timeline Visual** | ✅ **COMPLETO** (aguarda smoke E2E + PR) | Componente 12 (atualizacao v4.0) — **4ª e ULTIMA entrega da Wave 3 v4.0** (de 4 — C10, C19, C11 ja entregues). Fechando o C12, a Wave 3 inteira fica pronta para merge `development → main`. Frontend-only, zero touch em backend, RLS ou migrations. Gate-based two-stage: 11 decisoes de design escaladas ao Mario (sem Figma anexado — proposta apresentada em ASCII wireframes no `analysis.md` §5; recomendacoes tecnicas seguidas com Mario alterando apenas a Decisao 11). **Adicionado**: Timeline visual reformulada ([Timeline.tsx](frontend/src/app/(dashboard)/provas/%5Bid%5D/Timeline.tsx) 561 LOC reais pos-AUD-001 da auditoria — era documentada como 410, divergencia reconciliada em AUD-W3C12-002; era 273 antes do C12) + [timeline.module.css](frontend/src/app/(dashboard)/provas/%5Bid%5D/timeline.module.css) 471 LOC reais (era documentada como 372; era 211 antes do C12); modulo puro [lib/timeline-builder.ts](frontend/src/lib/timeline-builder.ts) 354 LOC reais (era documentado como 240) com pipeline `buildTimeline(prova, movimentacoes) -> BuiltTimeline`; helpers em `lib/types/prova.ts` (`ContextoMotorista`, `contextoMotorista`, `ESTADOS_LAMINACAO`, `isInLaminationBlock`, `ROTA_ETAPAS`, `LEGACY_ROTA_PADRAO`, `LEGACY_ROTA_DIRETA`, `getRotaEtapas`, `getRotaLabel`). **Features**: badge da rota no header + ícone terminal/cancelado; bloco visual "Etapa de Laminação" envolvendo nos adjacentes com borda tracejada verde #c0ca33 (ADR-160); 3 contextos do motorista com badges textuais "→ Laminação"/"Laminação →"/"→ Clicheria" (paridade Python `contexto_motorista`); multiplos ciclos empilhados com separador "↻ reinício de ciclo" + header diferenciado (atual/reprovado); etapas pendentes com dot outline + conector tracejado + label "Aguardando" (ADR-161); card transversal vermelho de cancelamento com alert-triangle + ator + motivo + timestamp; badge "Concluída" + check verde no terminal OK; indicador atual = dot amarelo + box-shadow + pulse framer-motion + badge "Atual"; respeita `useReducedMotion` (RNF-010). **Decisao 11.1**: `ROTA_LABELS["PADRAO"]="Matriz"` e `["DIRETA"]="Filial"` globalmente (supersede ADR-126; ADR-158); mudanca de 2 strings em `prova.ts` propaga para detalhe + listagem + relatorios + CSV (R-12: filtros do C07 podem ter duplicacao visual — a confirmar pos-merge). **Decisao 11.2**: heuristica `vendedor_localizacao → rota visual` para `rota=NULL` (ADR-159; 11/11 provas em producao com rota=NULL tem vendedor FILIAL → label "Filial" + sequencia LEGACY_ROTA_DIRETA). **Decisao 11.3**: bloco de laminacao NUNCA para legacy. **a11y AA**: `role="region"/list/listitem/group/alert"` + `aria-current="step"` + `aria-label` descritivo + dupla defesa `useReducedMotion` JS + `@media prefers-reduced-motion` CSS. **Icones SVG inline** (padrao do projeto, sem lucide-react): `CheckCircleIcon`/`AlertTriangleIcon`/`BanIcon`. **Testes Vitest**: **163 passed** (era 98 + 65 novos — 45 em prova.test.ts + 20 em timeline-builder.test.ts cobrindo 4 rotas v4.0 + 5 cenarios legacy + multiplos ciclos + cancelamento + 3 contextos + edge cases). **Validacao**: tsc 0; next build 13/13; `/provas/[id]` em **16.1 kB / 214 kB** (era 11.4/209 — overhead +4.7 kB pelo redesign + framer-motion expandido); advisors MCP sem novos alertas; smoke programatico `/login` 0 erros (Timeline em si requer auth — smoke E2E manual em [docs/wave3-v4-c12/smoke-validation.md](docs/wave3-v4-c12/smoke-validation.md)). ADRs 158-161. Documentos: [analysis.md](docs/wave3-v4-c12/analysis.md) com Gate 1 completo (inventario contrato + Timeline + 11 decisoes + 8 ASCII wireframes + Apendice de Execucao) + [smoke-validation.md](docs/wave3-v4-c12/smoke-validation.md). **FECHA A WAVE 3 v4.0**. | — |
| **v4.0 W5 — C16 Audit Fixes** | ✅ **COMPLETO** (aguarda smoke E2E + PR + re-auditoria) | Auditoria sênior independente pós-C16 (2026-05-13, branch `wave5-v4-c16/audit` HEAD `57a76d2`). Veredito: **APROVADO COM CORREÇÕES**. Findings: **0 CRITICO · 3 ALTO · 5 MEDIO · 5 BAIXO · 4 INFO = 17 totais**. Sessao de correcao 2026-05-13 (`wave5-v4-c16/fixes/execution` base `wave5-v4-c16/audit`) tratou TODOS os 17 IDs em **11 commits atomicos + 1 commit do plano**. **RESOLVIDOS — codigo tocado (12)**: ALTOS (3): AUD-001 (visual-guide.md criado 227 LOC, padrao Wave 3 C12 AUD-W3C12-003), AUD-002 (smoke-validation expandido 20->23 cenarios — +#21 vazio +#22 erro +#23 acesso negado), AUD-003 (`aria-hidden="true"` movido de `<details>` para `<table>` interna no DonutChart — WAI-ARIA 1.1 §4.3.2 conforme, `<summary>` permanece focavel). MEDIOS (5): AUD-004 (`_CONTEXTO_MOTORISTA_STATUSES` derivado por dict comprehension da fonte canonica `app.state_machine.v4.contextos.contexto_motorista`; teste cross-validation novo itera sobre os 17 valores de StatusProvaEnum), AUD-005 (bloco `@media (prefers-reduced-motion: reduce)` em relatorios.module.css cobrindo 8 seletores), AUD-006 (parsers extraidos para `frontend/src/hooks/_useReportFilters.parsers.ts` modulo puro; hook + testes importam do modulo — sem re-implementacao), AUD-007+008 (renomear `.rotaDotPadrao`->`.rotaDotMatriz`, `.rotaDotDireta`->`.rotaDotFilial` + bloco de comentario documentando mapeamento v3->v4 e citando ADR-158). BAIXOS (2): AUD-010 (classe TestLegacyNullIndefinida com 4 testes cobrindo null_indef = null_total - null_matriz - null_filial e schema ConsolidacaoRota.indefinida >= 0), AUD-011 (guard `if cons.indefinida > 0` removido em `_summary_rows` — linha CSV `consolidacao_rota_indefinida` agora sempre emitida; classe TestCsvSummaryConsolidacaoIndefinida com 3 testes). **ACEITOS sem codigo (5)**: AUD-009 (docstring `_CLICHERIA_EM_TRANSITO` ja cobre — auditor confirmou), AUD-012/013 (INFO de revisao), AUD-014/015/016 (INFO de cobertura), AUD-017 (anti-enumeracao 403 — Decisao D11→i registrada em ADR-162 com 5 justificativas; **Apendice 2 ao ADR-162** reafirma posicao pos-auditoria; follow-up registrado para Wave 6+ se Mario quiser migrar Matriz inteira para 404 byte-a-byte). **DEFERRED**: 0. **BLOQUEADOS**: 0. **Nova escalacao humana**: 0. **Validacao**: pytest backend **1035 passed + 10 skipped** (era 1027 + 8 novos: 1 AUD-004 + 4 AUD-010 + 3 AUD-011, sem regressao); frontend Vitest **205 passed** (testes do AUD-006 agora exercem codigo real do modulo extraido); npx tsc --noEmit exit 0; npx next build **13/13** paginas; bundle `/relatorios` mantem ~17.9 kB / ~220 kB (sem regressao); `git diff origin/development..HEAD` em paths protegidos (`contrato-c12.md`, dashboard/, maquina v4, state_machine, escanear, provas, nova-prova, RLS, migrations, Wave 1 RBAC, middleware, services frontend): **VAZIO**; MCP advisors security + performance **identicos ao baseline pos-C16**. **Pendencias para PR `development → main`** (mantidas): rate limit C19 (ADR-145), benchmarks C11 (ADRs 153/157), CI/CD pos-Wave 3 (ADR-156). Especificas C16 Audit Fixes: smoke E2E manual do Mario (23 cenarios) + nova rodada de auditoria senior independente. **MARCO**: ultima sessao de correcao de componente da Wave 5 v4.0. Recomendado: nova rodada de auditoria independente em sessao separada + sessao de revisao consolidada pre-merge (Wave 3 + Wave 5 juntas) antes de `development → main`. ADRs novos: nenhum. **Apendice 1+2 ao ADR-162** em DECISIONS.md documenta a sessao + reafirma Decisao D11→i. Documentos: [fix-plan.md](docs/wave5-v4-c16/fix-plan.md) (729 LOC), [visual-guide.md](docs/wave5-v4-c16/visual-guide.md) (227 LOC), [smoke-validation.md](docs/wave5-v4-c16/smoke-validation.md) (23 cenarios), [audit-report.md com apendice de status](docs/wave5-v4-c16/audit-report.md), [fix-validation.md](docs/wave5-v4-c16/fix-validation.md). | — |
| **v4.0 W5 — C16 Relatorios v4.0** | ✅ **COMPLETO** (aguarda smoke E2E + PR) | Componente 16 (atualizacao v4.0) — **1a e UNICA entrega da Wave 5 v4.0** (C17 rebaixado a "sem alteracao" mantem v3). Fechando o C16, a Wave 5 v4.0 fica pronta para revisao pre-merge `development → main`. **Restricao chave do Mario:** "Vamos seguir sua recomendacao, desde que nao altere nada no layout que temos hoje em dia." Resposta: backend estendido aditivamente; frontend muda apenas semantica interna; zero novos elementos visiveis. **Backend:** 4 schemas Pydantic novos (`DistRotaV4` 9 categorias + `ConsolidacaoRota` matriz/filial/indef + `DistContextoMotorista` 3 contextos + `RotaCategoria` Literal); filtro `rota_categoria` em `ReportFilters` (precedencia sobre `rota` exata); helpers `_ROTAS_MATRIZ`/`_ROTAS_FILIAL` (baldes disjuntos cobrindo as 6 rotas), `_CONTEXTO_MOTORISTA_STATUSES` (paridade byte-a-byte com `contextoMotorista()` TS e `contexto_motorista()` Python), `_categoria_predicate()` (gera SQL `rota IN {...} OR (rota IS NULL AND vendedor.localizacao=...)` via correlated EXISTS — heuristica C12 D11.2), `_contexto_motorista_csv()`; Q1 do `_aggregate_geral` expandida para 9 contadores de rota + 2 sub-buckets NULL; Q7 nova snapshot contexto motorista; `_aggregate_clicheria` semantica v4.0 (via_padrao + COM_MOTORISTA_ENTREGA_FINAL; via_direta + APROVADA→RECEBIDA direto para FILIAL/LAM_FILIAL); CSV `proofs` ganha `codigo_publico` + `contexto_motorista`; `overdue` ganha `contexto_motorista`; `summary` ganha `rota_v4_*` + `consolidacao_rota_*` + `contexto_motorista_*`; `audit_logs.detalhes_json` em `REPORT_EXPORTED` inclui `rota_categoria`. **Frontend:** tipos `RotaCategoria`/`DistRotaV4Item`/`ConsolidacaoRota`/`DistContextoMotoristaItem` em `report.ts`; `useReportFilters.parseRota` aceita 6 valores (importa `ROTA_OPTIONS`), `parseStatus` aceita 17 (importa `STATUS_OPTIONS` — corrige bug silencioso onde `?status=COM_MOTORISTA_IDA_LAMINACAO` era zerado), novo `parseRotaCategoria`; `RotaFilter.tsx` reescrito (3 botoes visualmente identicos Todas/Matriz/Filial, opera sobre `RotaCategoria`); `relatorios/page.tsx` passa `filters.rota_categoria` ao filtro; `ReportGeral` card ROTA usa `consolidacao_rota.matriz/filial` com fallback para `distribuicao_rota` legacy (labels "Padrao"→"Matriz", "Direta"→"Filial"); tabela `<table class="srOnly">` permanente no `DonutChart` (D7 Opcao ii — leitor de tela acessa imediatamente; `<details>` interno marcado `aria-hidden=true` para nao duplicar). **Nao-modificado:** estrutura geral (tabs + 4+3 cards), C15 Dashboard, Report3Studio/Vendedores/Clicheria visualmente, DateRangeFilter/SearchInput/StatusFilter visuais, Recharts continua removido (SVG inline preservado da W4). **Migrations:** ZERO Alembic + ZERO RLS (banco ja suportava — `idx_provas_rota` existe, enum 17 valores). **Testes:** backend **1027 passed + 10 skipped** (era 967 + 60 novos em `test_reports_v4.py` cobrindo 6 classes); frontend Vitest **205 passed** (era 163 + 42 novos em `useReportFilters.test.ts`). tsc 0; next build 13/13; `/relatorios` 17.9 kB / 220 kB (era 11.4/200 — overhead +6.5 kB pelos campos novos + tabela sr-only + logica de fallback); advisors MCP identicos ao baseline pos-C12. ADR **162** (preservar layout v3 + adaptacao das 11 decisoes do Gate 1). Documentos: [analysis.md](docs/wave5-v4-c16/analysis.md) (Gate 1 + Apendice A de execucao), [smoke-validation.md](docs/wave5-v4-c16/smoke-validation.md) (20 cenarios). **Pendencias para PR `development→main`** (herdadas Wave 3): rate limit C19 (ADR-145), benchmarks C11 (ADRs 153/157), CI/CD pos-Wave 3 (ADR-156); especificas C16: smoke E2E manual do Mario + auditoria senior. **FECHA A WAVE 5 v4.0.** | — |
| **v4.0 W3 — C12 Audit Fixes** | ✅ **COMPLETO** (mergeado em `development` em 2026-05-13) | Auditoria sênior independente pos-C12 (2026-05-13, branch `wave3-v4-c12/audit` HEAD `2a18794`). Veredito: **APROVADO COM CORREÇÕES MENORES**. Findings: **0 CRITICOS · 0 ALTOS · 4 MEDIOS · 5 BAIXOS · 4 INFO = 13 totais**. Sessao de correcao 2026-05-13 (`wave3-v4-c12/fixes/execution` base `wave3-v4-c12/audit`) tratou **TODOS os 13 IDs em 11 commits atomicos** + apendice formal a Decisao 7 em DECISIONS.md (sem novo ADR — registro de decisao consciente). **CORRIGIDOS (8)**: AUD-001 (remove `<AnimatePresence>` sem motion children — bonus: bundle `/provas/[id]` baixou 16.1->14.4 kB / -1.7 kB / -2 kB First Load); AUD-002 (reconcilia LOCs reais em 4 docs CLAUDE/CHANGELOG/PR/analysis — Timeline.tsx 561 LOC reais era documentado 410, divergencia de 151 LOC corresponde a 73 SVG icons inline + 22 JSDoc + ~60 subcomponentes inline; tambem reconcilia CSS 471 vs 372, builder 354 vs 240; analysis.md §17.6 nova "Apêndice à §17.5" preserva ata original); AUD-003 (cria `docs/wave3-v4-c12/visual-guide.md` stub 319 LOC com 8 secoes estruturadas + provas representativas por cenario + placeholders para screenshots do Mario pos-smoke); AUD-004 (captura coverage snapshot pontual em `docs/wave3-v4-c12/coverage-snapshot.md` via `npm install --no-save @vitest/coverage-v8@~2.1.0` + uninstall apos — preserva D-13 da Wave 1 v4.0 sem persistir dep; coverage: timeline-builder.ts 99.46% lines / 100% funcs / 93.33% branch; prova.ts 94.94% lines / 100% funcs do C12; global agregado 97.15% — confortavelmente acima do limiar 80% do criterio 19); AUD-005 (`<aside role="alert">` -> `<div role="alert">` no CancellationCard — `<aside>` significa "side content" HTML5; `role="alert"` preservado dispara anuncio); AUD-007 (remove `role="list"` redundante de 3 ocorrencias `<ol>`/`<ul>` em Timeline.tsx — role implicito por HTML5 preservado); AUD-010 (remove `aria-label` redundante do rotaBadge — texto interno ja eh lido). **REBAIXADO + RESOLVIDO via Opcao A (1)**: AUD-006 (Decisao 7 parcial — sem tachado nos passos anteriores ao cancelamento). Mario aprovou Opcao A em 2026-05-13: aceitar como esta. Apendice formal a Decisao 7 em DECISIONS.md apos ADR-161 documenta 4 justificativas: (1) honestidade historica — tachado significa "deletado/anulado" e a movimentacao anterior aconteceu de fato gravada com trigger de imutabilidade; (2) os 3 mecanismos atuais (card vermelho `role="alert"` + no cinza CANCELADA + motivo destacado) preservam a essencia da Decisao 7 sem ambiguidade; (3) a11y equivalente — `role="alert"` ja dispara anuncio; (4) principio "do no harm". Custo: 0 LOC de codigo + 58 LOC de apendice. **DEFERRED (3)**: AUD-008 (performance < 500ms — smoke 15 manual Mario; estimativa auditor < 50ms folga 10x); AUD-009 (snapshot/E2E aceito como tradeoff D-13 documentado em analysis §17.3.1); AUD-011 (R-12 filtros C07 com duplicacao visual "Matriz × 2" / "Filial × 2" apos Decisao 11.1 ADR-158 — decisao pos-merge). **ACEITOS (2)**: AUD-012 (consolidado com AUD-009); AUD-013 (subcomponentes inline em Timeline.tsx — decisao consciente analysis §17.3.2). **Validacao**: npx vitest run **163/163** (sem regressao); npx tsc --noEmit exit 0; npx next build **13/13** paginas; `/provas/[id]` em **14.4 kB / 212 kB** (era 16.1/214 — Δ -1.7 kB pelo AUD-001); MCP advisors security + performance **identicos ao baseline pos-C11** (1 INFO `alembic_version` + 1 WARN `auth_leaked_password` security; 13 INFO `unused_index` performance — todos pre-existentes); `git diff origin/development..HEAD --` em paths protegidos (backend/, contrato-c12.md, backend/app/state_machine/, escanear/, AdminActions, page.tsx detalhe, detalhe.module.css, services/, codigo-publico, c19-mensagens, middleware, shared/, useProvaDetail): **VAZIO**. **Pendencias para PR em `main` (mantidas)**: rate limit backend (ADR-145 do C19) + benchmarks (ADRs 153/157 do C11) + CI/CD pos-Wave 3 (ADR-156 do C11) — sessoes dedicadas. Especificas C12: smoke E2E manual 18 cenarios (2/3/4 SKIP em producao R-4); validacao leitor de tela + axe-core manual + performance medida (smokes 12/14/15); screenshots para visual-guide.md; decisao R-12. **MARCO**: ultima sessao de correcao de componente da Wave 3 v4.0. Recomendado: nova rodada de auditoria independente em sessao separada (foco extra: rebaixamento AUD-006 + paridade visual das 14 decisoes + smoke E2E pelo Mario) + sessao de revisao de Wave 3 inteira pre-merge `main` (C10 + C19 + C11 + C12 + Audit Fixes). Documentos: [fix-plan.md](docs/wave3-v4-c12/fix-plan.md), [fix-validation.md](docs/wave3-v4-c12/fix-validation.md) (com auto-critica adversarial 21 perguntas), [visual-guide.md](docs/wave3-v4-c12/visual-guide.md) (stub), [coverage-snapshot.md](docs/wave3-v4-c12/coverage-snapshot.md), [audit-report.md com apendice de status pos-correcao](docs/wave3-v4-c12/audit-report.md), [pr-fixes-description.md](docs/wave3-v4-c12/pr-fixes-description.md). | — |
| **v5.0 W8 — C22 Reativacao da Tela de Assinatura** | ✅ **COMPLETO** (aguarda smoke E2E manual + PR + auditoria) | Componente 22 (novo na v5.0) — **1a entrega da Wave 8 v5.0 e da v5.0 como um todo**. Reativacao da tela de assinatura no fluxo de escaneamento (RF-028, RN-014): o frontend de assinatura foi descontinuado no redesenho do C10 v4.0 (commit `e4d543b`); o backend permaneceu integralmente operacional. Gate-based two-stage com fase de arqueologia de codigo. **Gate 1**: arqueologia ([docs/wave8-v5-c22/arqueologia.md](docs/wave8-v5-c22/arqueologia.md)) recuperou a UI removida verbatim do commit-fonte `6add246`; analise read-only ([analysis.md](docs/wave8-v5-c22/analysis.md)) — descobriu que **C20 (animacoes) e C21 (migracao de dados) NAO existem no codigo** (o prompt assumia entregues); Mario decidiu mante-los pendentes e prosseguir com o C22. 11 decisoes de design + Q1/Q2 escaladas e aprovadas. **Gate 2**: `AssinaturaModal` reconstruido (modal sobre `/escanear`, abre automaticamente apos a identificacao quando `transicoes_permitidas` nao-vazio — RF-028); `CapturaAssinatura` (wrapper `react-signature-canvas`, canvas responsivo); `lib/assinatura/helpers.ts` (labels de transicao v4.0 dos 17 estados + deteccao do proximo ator + contexto do motorista); `assinatura.module.css` (classes migradas verbatim da arqueologia, touch targets >=44px); hook `useExecutarTransicao` reativado (orfao desde o C10 v4.0) + campo `status`. Integracao leve no `escanear/page.tsx` (2 pontos pos-identificacao -> regra unica `handleIdentificada`). **Regra unica:** scan -> se voce e o proximo ator, assina antes; senao, vai direto a `/provas/[id]`. Seletor Aprovar/Reprovar para o vendedor (RF-008); motivo da reprovacao no mesmo modal; tratamento de 409 (race), 401 (sessao), falha de rede (retry com assinatura preservada em memoria). Compativel com as 4 rotas v4.0, provas legacy v3.0 e os 3 contextos do motorista. **Zero backend / migration / RLS / endpoint novo.** Bundle `/escanear` 8.31->15.9 kB Size / 210->220 kB First Load. **222 testes Vitest** (era 205 + 17 novos); tsc 0; next build 13/13; next lint 0 warnings; advisors MCP identicos ao baseline. ADRs 163 (11 decisoes) + 164 (ator-errado in-scope abre `/provas/[id]` — revisao de RN-014/Cenario 4 com chancela explicita do Mario). Pendencias: smoke E2E manual dos 10 cenarios ([smoke-validation.md](docs/wave8-v5-c22/smoke-validation.md)) + auditoria senior + redeploy do Railway (estava fora do ar no Gate 1). | — |
...
## Tela de Assinatura: fluxo, integracao e RBAC (Wave 8 v5.0 / Componente 22)

A tela de assinatura foi **reativada** no fluxo de escaneamento. Ela
existia antes do redesenho do C10 v4.0 (que a removeu); o C22 a reconstroi
a partir da arqueologia (commit-fonte `6add246`). O backend de assinatura
nunca foi tocado — esta entrega e exclusivamente frontend.

### Fluxo (regra unica — ADR-163 / ADR-164)

Apos o `/escanear` identificar uma prova (camera do C10 OU digitacao
manual do C19), o callback `handleIdentificada(scan)` decide:

- `scan.transicoes_permitidas` **nao-vazio** -> o usuario logado E o
  proximo ator habilitado -> o **modal de assinatura abre automaticamente**
  (RF-028).
- `scan.transicoes_permitidas` **vazio** -> nao e a vez dele (ou prova
  terminal) -> navega para `/provas/[id]` (Decisao D6 / ADR-164).
- Prova fora do escopo RLS -> o `/scan` ja retornou 404 -> a pagina exibe
  o erro generico (anti-enumeracao — comportamento inalterado).

Toda saida do modal (sucesso, cancelamento, conflito, sessao) navega para
`/provas/[id]`. Resumo: *scan -> se voce e o ator, assina antes; senao,
vai direto ao detalhe.*

### Como invocar / estender

O modal e auto-disparado pelo `escanear/page.tsx` — nao ha invocacao
manual noutras paginas. Para reusar a tela de assinatura num fluxo futuro:

```tsx
import { AssinaturaModal } from "@/components/assinatura/AssinaturaModal";
import { deveAbrirAssinatura } from "@/lib/assinatura/helpers";

// scan: ScanResponse (de POST /scan, via identificacao-prova.ts)
if (deveAbrirAssinatura(scan)) {
  // <AssinaturaModal scan={scan} getToken={getToken}
  //   onFechar={() => router.push(`/provas/${scan.prova.id}`)} />
}
```
...
|---|---|
| `frontend/src/components/assinatura/AssinaturaModal.tsx` | Modal: seletor Aprovar/Reprovar, captura, motivo, submit, views de resultado |
| `frontend/src/components/assinatura/CapturaAssinatura.tsx` | Wrapper do `react-signature-canvas` (canvas responsivo) |
| `frontend/src/components/assinatura/assinatura.module.css` | CSS Module do modal |
| `frontend/src/lib/assinatura/helpers.ts` | Helpers puros (labels v4.0, deteccao, contexto) |
| `frontend/src/hooks/useExecutarTransicao.ts` | POST `/{id}/transicoes` (reativado; retorna `status`) |

### Backend (NAO tocar — consumido como esta)

- `POST /api/v1/provas/scan` -> `ScanResponse { prova,
  transicoes_permitidas, motivo_obrigatorio_em }`. `transicoes_permitidas`
  ja vem filtrado pelo backend para o usuario corrente (maquina v4.0 ou
  v3.0 — `_computar_transicoes_permitidas`).
- `POST /api/v1/provas/{id}/transicoes` -> recebe `TransicaoRequest
  { status_novo, assinatura_base64, motivo_reprovacao? }`; 201 sucesso,
  409 race, 422 generico, 401 sessao.

### RBAC

- `/escanear` e acesso universal (Matriz Secao 6 — rule `scanner`, 4
  perfis full). A validacao "este perfil pode assinar esta prova?" e feita
  pelo backend via `transicoes_permitidas` — o C22 NAO reimplementa a
  maquina de estados.
- Anti-enumeracao (RN-014): ver ADR-164. O limite de enumeracao continua
  sendo o escopo RLS + o 404 generico do `/scan`.

### Decisoes

ADR-163 (11 decisoes de design) + ADR-164 (ator-errado in-scope). Detalhe
em `docs/wave8-v5-c22/analysis.md`.
