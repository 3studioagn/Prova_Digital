...
│   │   │   └── models.py        # Usuario model, SetorEnum, LocalizacaoEnum
│   │   ├── api/
...
│       └── test_users_api.py    # 25 testes integracao endpoints
├── frontend/
...
│       ├── hooks/useInactivityTimeout.ts  # Timer 30min (RNF-003)
│       ├── lib/
│       │   ├── api.ts           # apiFetch wrapper (token injection, ApiError)
│       │   └── supabase/
