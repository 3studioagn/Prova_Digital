...
| AUD-W1V4-107 | RESOLVIDO | _pending_ (via AUD-003) | A reescrita da etapa [4/4] feita em AUD-W1V4-003 substituiu o comentário enganoso "M-5: asserça de verdade" pelo bloco novo que cita explicitamente "AUD-W1V4-003" e "AUD-W1V4-107" descrevendo a cobertura real (validação por `(rule, profile, table)` triple). Grep `asser.a de verdade` no script: 0 hits. Docstring de cabeçalho do script já lista as 6 tabelas no procedimento. |
| AUD-W1V4-106 | RESOLVIDO | _pending_ | `DECISIONS.md` ganha apêndice **D-8 — `_scoping_filter` mantido como shim (status formal de L-2)** explicando o trade-off: refator de 7 chamadas é fora do escopo desta sessão de RBAC; o comentário in-code já documenta o caráter de shim; o AUD-W1V4-105 criou `_scoping_filter_for_detail` para o caminho de detalhe, e eliminar o shim de listagem agora criaria 2 helpers inconsistentes. Status quo aceito conscientemente. |
| AUD-W1V4-201 | RESOLVIDO | _pending_ | `DECISIONS.md` apêndice **D-9 — Invariante: `dashboard` deve permanecer FULL para os 4 perfis** registrado. Toda mudança futura na Matriz que altere `dashboard` para algo diferente de `full` precisa, no mesmo PR, atualizar `home_by_profile` para não apontar para `/dashboard` no perfil afetado. Validação automática (teste) registrada como follow-up técnico. |
| AUD-W1V4-202 | RESOLVIDO | _pending_ | `DECISIONS.md` apêndice **D-10 — "Registro órfão invisível" aceito como improvável** registrado. Cenário arquiteturalmente impossível sob FK ON DELETE RESTRICT + triggers de imutabilidade. Premissa documentada para revisores futuros. |
| AUD-W1V4-203 | RESOLVIDO | _pending_ | `DECISIONS.md` apêndice **D-11 — Mudanças de RLS rastreadas via `supabase_migrations` + Git** registrado. `audit_logs` é log de domínio; mudanças DDL ficam em `supabase_migrations` (Supabase) + commits Git em `backend/migrations/rls/*.sql`. Sem duplicação. |
| AUD-W1V4-204 | RESOLVIDO | _pending_ | `DECISIONS.md` apêndice **D-12 — Extracts dos `.docx` removidos pos-Gate 1** registrado. Reprodutibilidade preservada via citações textuais em `analysis.md`, `EXPECTED_KEYS` em testes, `_clicheria_divergence_note` no JSON, e `.docx` originais em Desktop/Rastreio Prova Digital/. Restaurar extracts foi rejeitado como duplicação. |
